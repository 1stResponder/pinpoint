            ...
            try
            {
                ...
                switch (values.Length)
                {
                    ...
                    case 2: // Hours and decimal minutes
                        ...
                        if (values[0].IndexOf(culture.NumberFormat.NumberDecimalSeparator, StringComparison.CurrentCulture) != -1)
                        {
                    ...
                    default: // Hours, minutes and seconds  (most likely)
                        // If this is a fractional value, remember that it is
                        if (values[0].IndexOf(culture.NumberFormat.NumberDecimalSeparator) != -1 || values[0].IndexOf(culture.NumberFormat.NumberDecimalSeparator, StringComparison.CurrentCulture) != -1)
                        {
                            throw new ArgumentException(Resources.Angle_OnlyRightmostIsDecimal, "value");
                        }
...
Math.Round(
        ...
        public string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            try
            {
                ...
                if (format.IndexOf(culture.NumberFormat.NumberDecimalSeparator, StringComparison.CurrentCulture) !=
                    format.LastIndexOf(culture.NumberFormat.NumberDecimalSeparator, StringComparison.CurrentCulture))
                    throw new ArgumentException(Resources.Angle_OnlyRightmostIsDecimal);
                // Is there an hours specifier?
                int startChar = format.IndexOf("H", StringComparison.CurrentCulture);
                int endChar;
                if (startChar > -1)
                {
                    // Yes. Look for subsequent H characters or a period
                    endChar = format.LastIndexOf("H", StringComparison.CurrentCulture);
                    ...
                    // Replace the hours
                    if (newFormat.IndexOf(culture.NumberFormat.NumberDecimalSeparator, StringComparison.CurrentCulture) > -1)
                    {
                ...
                // Is there an hours specifier°
                startChar = format.IndexOf("M", StringComparison.CurrentCulture);
                if (startChar > -1)
                {
                    // Yes. Look for subsequent H characters or a period
                    endChar = format.LastIndexOf("M", StringComparison.CurrentCulture);
                    // Extract the sub-string
                    subFormat = format.Substring(startChar, endChar - startChar + 1);
                    // Convert to a numberic-formattable string
                ...
                // Is there an hours specifier°
                startChar = format.IndexOf("S", StringComparison.CurrentCulture);
                if (startChar > -1)
                {
                    // Yes. Look for subsequent H characters or a period
                    endChar = format.LastIndexOf("S", StringComparison.CurrentCulture);
                    // Extract the sub-string
                    subFormat = format.Substring(startChar, endChar - startChar + 1);
                    // Convert to a numberic-formattable string
                ...
                // Is there an hours specifier?
                startChar = format.IndexOf("C", StringComparison.CurrentCulture);
                if (startChar > -1)
                {
                    // Yes. Look for subsequent H characters or a period
                    endChar = format.LastIndexOf("C", StringComparison.CurrentCulture);
                    // Extract the sub-string
                    subFormat = format.Substring(startChar, endChar - startChar + 1);
